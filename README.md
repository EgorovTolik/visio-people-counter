...
«старые → новые координаты». Size-точки — 2D: клик запоминает позицию человека в кадре
(и x, и y), цифра задаёт его рост; в конфиге это тройки `[x, y, h]` в долях кадра
...
`−точка` (клавиша `b`, только в режиме «размер»: отменяет ПОСЛЕДНЮЮ size-точку;
ошибочную точку можно удалить и кликом прямо по ней — hit-радиус ~15 px на экране; из
других режимов кнопка/клавиша подсказывают включить режим размер),
...
клавишам `[` / `]` (циклически). Можно настроить несколько линий и несколько зон: переключайтесь
...
| `s`, клик, цифра 1–9 | size-точка: клик задаёт точку кадра (важен и x, и y), высота человека = цифра×5% высоты кадра (1=5% … 9=45%); Enter — завершить набор; **клик по уже нарисованной size-точке (~15 px) удаляет её** |
...
| `b` | отменить ПОСЛЕДНЮЮ size-точку (только в режиме «размер»; то же, что кнопка «−точка»); если точек нет — уведомление «размер: точек нет» |
